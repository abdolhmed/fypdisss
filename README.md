...
|---|---|
| `geocoding.ipynb` | Initial data collection and processing - scrapes and parses raw property listings from a `.txt` file sourced from a real estate agent, merges with the Property Price Register, and uses the Google Maps API to enrich each record with coordinates and distances to key locations. |
| `main.ipynb` | Core analysis notebook — merges all collected datasets, performs preprocessing (cleaning, imputation, feature engineering), and trains/evaluates all machine learning models |
